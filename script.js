...
const festivalDays = [
  ...
  { id: "ashtami", bn: "অষ্টমী", name: "Ashtami", subtitle: "The Divine Power", message: "Anjali, incense and a sea of diyas honour the radiant strength of Maa Durga.", date: new Date(2026, 9, 19), arrived: "Shubho Maha Ashtami", image: "images/ashtami.jpg", accent: "#ff6863", rgb: "255,104,99" },
  { id: "navami", bn: "নবমী", name: "Navami", subtitle: "The Celebration of Victory", message: "Festival lights rise through the evening as joy, music and devotion fill the pandal.", date: new Date(2026, 9, 20), arrived: "Shubho Maha Navami", image: "images/navami.jpg", accent: "#ec65f5", rgb: "236,101,245" },
  { id: "dashami", bn: "দশমী", name: "Dashami", subtitle: "The Farewell", message: "A golden farewell by the river, with sindoor, blessings and the promise Maa Durga will return.", date: new Date(2026, 9, 21), arrived: "Subho Bijoya", image: "images/dashami.jpg", accent: "#ff944f", rgb: "255,148,79" }
];

const songs = [
  ["Mahalaya — Original Chandi Path", "file:///C:/Users/partha/Downloads/Mahalaya%20__%20Original%20Chandi%20Path%20__%20Birendra%20Krishna%20Bhadra%20full%20Chandipath%20%23%E0%A6%AE%E0%A6%B9%E0%A6%BF%E0%A6%B7%E0%A6%BE%E0%A6%B8%E0%A7%81%E0%A6%B0%E0%A6%AE%E0%A6%B0%E0%A7%8D%E0%A6%A6%E0%A7%8D%E0%A6%A6%E0%A6%BF%E0%A6%A8%E0%A7%80%20%5BYQFNRoi7rEc%5D.mp3"],
  ["Gouri Elo Dekhe Ja Lo", "audio/Gouri-elo-dekhe-ja-lo-mp3-the-bongclub.mp3"],
  ["Amar Dugga Maa", "audio/amar-dugga-maa-mp3-the-bongclub.mp3"],
  ...
  ["Bochor Bochor Aste Hobe", "audio/bochor-bochor-aste-hobe-mp3-the-bongclub.mp3"],
  ["Dhak Baja Kashor Baja", "audio/Dhak-Baja-Kashor-Baja-Mp3-the-bongclub.mp3"],
  ["O Menoka O Menoka", "audio/o-menoka-o-menoka-mp3-the-bongclub.mp3"],
  ["Ailo Uma Barite", "audio/ailo-uma-barite-mp3-the-bongclub.mp3"],
  ["Uma Ase Notun Saje", "audio/uma-ase-notun-saje-mp3-the-bongclub.mp3"],
  ["Bolo Bolo Dugga Alo", "audio/bolo-bolo-dugga-alo-mp3-the-bongclub.mp3"],
  ["Song 11 — add your file", ""], ["Song 12 — add your file", ""], ["Song 13 — add your file", ""],
  ["Song 14 — add your file", ""]
];
...
function render() {
  ...
  app.innerHTML = `<section class="sheet sheet--${day.id}" style="background-image:url('${day.image}');--accent:${day.accent};--accent-rgb:${day.rgb}">
    ...
    <div class="lower"><button id="previousDay" class="page-button" type="button" ${activeDay === 0 ? "disabled" : ""}>← Previous</button><p class="music-summary">15 Devotional Songs<br><button id="openPlaylist" type="button">Open playlist & play music</button></p><button id="nextDay" class="page-button" type="button" ${activeDay === 5 ? "disabled" : ""}>Next →</button></div>
  </section>`;
...
function renderPlaylist() {
  ...
  drawer.innerHTML = `<h2>15 Devotional Songs</h2><p>Select any available song. Replace filenames in the <code>songs</code> list in script.js later.</p><div class="playlist">${songs.map(([name, src], index) => `<button class="song ${index === activeSong ? "active" : ""}" data-song="${index}" type="button" ${src ? "" : "disabled"}><span class="song-number">${pad(index + 1)}</span><span><span class="song-name">${name}</span><span class="song-artist">${src ? "The Bong Club" : "Add your track"}</span></span></button>`).join("")}</div>`;
  drawer.querySelectorAll(".song").forEach(button => button.onclick = () => selectSong(Number(button.dataset.song), true));
}

function togglePlaylist() { drawer.classList.toggle("open"); }
...
audio.onplay = () => document.getElementById("playPause").textContent = "❚❚";
audio.onpause = () => document.getElementById("playPause").textContent = "▶";
